...
const Sidebar = () => {
	...
	return <nav className="min-h-screen" style={{ gridArea: "sidebar" }}  >
		<ul className=" flex flex-col h-full pl-4 gap-1 bg-sky-900 text-white ">
			...
			}} key={i}><Link className="h-full w-full p-4 inline-flex items-center hover:bg-sky-300 transition-colors ease-in" to={link.link}> {link.title}</Link>  </li>)}
		</ul>
