  ...
  return (
    <>
      <div
        ...
        className="relative min-h-24 pr-4 pl-4 w-full inline-flex gap-5 rounded-sm border-2 border-gray-400 items-center"
      ...
      >
        <button
          ...
          className="sticky right-0 top-0 z-200 "
        >
          <VscDiffRemoved className="w-6 h-6 fill-red-500 " />{' '}
        </button>
      ...
      {isOpen && (
        <Modal head="Add link" setIsOpen={setOpen}>
          <div className="w-full max-h-[80%] flex flex-col gap-4  justify-center items-center">
            ...
            />
            <ValidatedForm
              className="flex w-full justify-start h-lg flex-col rounded-sm gap-4  border-2 p-8 border-slate-400"
              validator={imageLinkValidator}
              method="post"
            >
              <p className="font-bold text-xl">Edit link</p>
              <FormInput name="isBaner" type="hidden" value={`${isBaner}`} />
              ...
              <SubmitButton classNames="border-2 rounded-sm border-blue-400 p-4 hover:bg-blue-200">
                Save
              </SubmitButton>
            </ValidatedForm>
          </div>
...
export const imageLinkValidator = withZod(
  z.object({
