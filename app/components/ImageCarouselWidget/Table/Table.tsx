...
const Table = ({ images, setSelectImage, selectImages }: {
	...
	const [newDataPosition, setDataPosition] = useState<(number | undefined)[] | null>()
	useEffect(() => {
		if (images.length !== 0) {
	...
	useDndMonitor({
		...
		onDragEnd(e) {
			...
			const draggedWidgetIndex = newDataPosition.findIndex((w) => w === active?.data?.current?.id);
			const overIndex = newDataPosition.findIndex((w) => w === over?.data?.current?.id);
