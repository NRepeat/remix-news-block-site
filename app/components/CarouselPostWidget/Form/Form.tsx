	...
	useEffect(() => {
		if (page) {
			...
			if (widgetInstanceContent) {
				const { postsIds, type, existCarouselId } = JSON.parse(widgetInstanceContent) as { postsIds: string, type: string, existCarouselId: string };
				const parsedPostsIds = typeof postsIds === "string" ? JSON.parse(postsIds) as number[] : postsIds;
				const filteredPosts = parsedPostsIds.map(id => posts?.find(post => post.id === id)).filter(post => post !== undefined) as SerializeFrom<PostWithTags>[];
	...
	return (
		<WidgetFormWrapper widget={widget}>
			...
			<div className={`flex w-full cursor-default justify-between gap-2 p-4 }  `}>
				<button className={`border-2 w-full rounded-sm pr-2 pl-2 ${option === 'popular' ? "bg-gray-500" : "bg-white"} `} onClick={(e) => handleOptionChange(e, 'popular')}>Popular</button>
				<button className={`border-2 w-full rounded-sm pr-2 pl-2 ${option === 'latest' ? "bg-gray-500" : "bg-white"} `} onClick={(e) => handleOptionChange(e, 'latest')}>Latest</button>
				<button className={`border-2 w-full rounded-sm pr-2 pl-2 ${option === 'manual' ? "bg-gray-500" : "bg-white"} `} onClick={(e) => handleOptionChange(e, 'manual')}>Manual selection</button>
			...
			<ValidatedForm className='p-4 pt-0 flex gap-2 flex-col justify-start items-start' validator={postCarouselFormValidator} method='post'   >
				...
				{option !== 'manual' &&
					<FormInput type="text" name="quantity" placeholder='Default 10 posts' label="Posts quantity" />
				}
