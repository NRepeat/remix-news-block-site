...
const ImageGrid: FC<ImageGridType> = ({
  images,
  selectedImage,
  setSelectedImage,
}) => {
  ...
  const handleDeleteImage = () => {
    if (selectedImage) {
      submit(
        {imageIdToDelete: selectedImage},
        {method: 'delete', navigate: false, action: '/admin/media'}
      );
    ...
    <div>
      {images && (
        <div className="flex gap-4 flex-wrap">
          {images.map(image => {
            const imagePath = image.path.includes('/')
              ? image.path
              : `/uploads/${image.path}`;
            return (
              <div key={image.id} className="relative ">
                {selectedImage === image.id && (
                  ...
                  >
                    <MdDelete className="fill-red-500 h-[30px] w-[30px] " />
                  </button>
                )}
                <img
                  aria-hidden
